...
public class UserApplicationCopy 
{  
  ...
  {  
    ...
    SensorCopy gasReader = new SensorCopy(userLocation);
    ...
    try{
      while (true){
        ...
        if (co2DailyAverages.size()==0){
          ...
          co2DailyAverages.set(idx, Double.valueOf(co2DailyAvg));
          so2DailyAverages.set(idx, Double.valueOf(so2DailyAvg));
          no2DailyAverages.set(idx, Double.valueOf(no2DailyAvg));
        }
        ...
        c.println(userLocation.toUpperCase() + ", ON\n");
